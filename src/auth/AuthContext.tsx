...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    window.electronAPI.getSession().then(session => {
      if (session) setUser({ id: session.email, name: session.email, coins: 0, streak: 0 })
      setAuthChecked(true)
    })
  ...
  return (
    <AuthContext.Provider value={{
      user, authChecked, hasCompletedOnboarding, onboardingChecked,
      login, signUp, confirmSignUp, signOut, completeOnboarding,
    }}>
      {children}
