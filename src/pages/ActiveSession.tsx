...
export default function ActiveSession({ nav, config }: Props) {
  ...
  const focusScore =
    captures.length === 0 ? 100 : Math.round((onTaskCount / captures.length) * 100);
  ...
  const pointsEarned = Math.max(0, Math.round(elapsedMinutes * (focusScore / 100) * 2));
    ...
    <div className="page">
      <div className={styles.container}>
        ...
        <div className={styles.header}>
          <span className={styles.pageTitle}>Active Session</span>
              ...
              >
                <span
                  className={`${styles.captureDot} ${
                    c.label === "off_task" ? styles.captureDotOff : styles.captureDotOn
                  }`}
                />
